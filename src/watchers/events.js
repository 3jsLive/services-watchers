...
const sql = {
	event: {
		insert: db.prepare( `INSERT OR IGNORE INTO events ( id, type, created_at, actor, changes ) VALUES ( $id, $type, $created_at, $actor, $changes )` )
	},
	actor: {
		insert: db.prepare( `INSERT OR IGNORE INTO actors ( id, login, avatar_url ) VALUES ( $id, $login, $avatar_url )` )
	},
	issue: {
		insert: db.prepare( `INSERT OR REPLACE
	...
	push: {
		insert: db.prepare( `INSERT OR REPLACE INTO pushs ( id, ref, head, before, size ) VALUES ( $id, $ref, $head, $before, $size )` ),
		log: db.prepare( `INSERT OR REPLACE INTO pushsLog ( eventId, id, timestamp ) VALUES ( $eventId, $id, $timestamp )` )
	},
	commit: {
		insert: db.prepare( `INSERT OR REPLACE INTO commits ( sha, push, author, message ) VALUES ( $sha, $push, $author, $message )` )
	}
};


function pullUserData( obj ) {
...
const handlers = {
	IssuesEvent: handlerIssuesEvent,
	IssueCommentEvent: handlerIssueCommentEvent,
	MilestoneEvent: handlerMilestoneEvent,
	PullRequestEvent: handlerPullRequestEvent,
	// PullRequestReviewEvent: handlerPullRequestReviewEvent,
	// PullRequestReviewCommentEvent: handlerPullRequestReviewCommentEvent,
	PushEvent: handlerPushEvent,
...
function processEvent( event, cache ) {
	...
	if ( typeof handlers[ event.type ] !== 'undefined' ) {

		sql.actor.insert.run( event.actor );
		...
		sql.event.insert.run( logData );

		result = handlers[ event.type ]( event, cache );
...
function handlerIssuesEvent( data/* , cache */ ) {
	...
	if ( validActions.indexOf( action ) !== - 1 ) {
		...
		if ( issue.milestone ) {
			...
			sql.milestone.insert.run( milestone );
		...
		sql.issue.log.run( { eventId: data.id, number: issue.number, action: action, parameter: JSON.stringify( parameter ), timestamp: data.created_at } );
...
function handlerIssueCommentEvent( data/* , cache */ ) {
	...
	if ( validActions.indexOf( action ) !== - 1 ) {
		...
		sql.actor.insert.run( issueActor );
		sql.actor.insert.run( commentActor );
		...
		sql.issue.insert.run( issueMeta );
		...
		sql.comment.insert.run( commentMeta );


		// log the actual action, current timestamp and parameter(s)
		let parameter = { actor: data.actor };
		sql.comment.log.run( { eventId: data.id, id: comment.id, action: action, parameter: JSON.stringify( parameter ), timestamp: data.created_at } );
...
function handlerMilestoneEvent( data/* , cache */ ) {
	...
	if ( validActions.indexOf( action ) !== - 1 ) {
		...
		sql.actor.insert.run( milestoneActor );
		...
		sql.milestone.insert.run( milestoneMeta );


		// log the actual action, current timestamp and parameter(s)
		let parameter = { actor: data.actor };
		sql.milestone.log.run( { eventId: data.id, id: milestone.number, action: action, parameter: JSON.stringify( parameter ), timestamp: data.created_at } );
...
function handlerPullRequestEvent( data/*, cache */ ) {
	...
	if ( validActions.indexOf( action ) !== - 1 ) {
		...
		// always add actors, in case we didn't know of them yet
		sql.actor.insert.run( pullUserData( pullrequest.user ) );
		sql.actor.insert.run( pullUserData( pullrequest.head.user ) );
		sql.actor.insert.run( pullUserData( pullrequest.base.user ) );
		sql.actor.insert.run( pullUserData( pullrequest.head.repo.owner ) );
		sql.actor.insert.run( pullUserData( pullrequest.base.repo.owner ) );

		if ( pullrequest.merged_by )
			sql.actor.insert.run( pullUserData( pullrequest.merged_by ) );

		if ( pullrequest.milestone )
			sql.actor.insert.run( pullUserData( pullrequest.milestone.creator ) );
		...
		sql.pullrequest.insert.run( pullrequestMeta );
		sql.issue.insert.run( issueMeta );
		...
		// log the actual action, current timestamp and parameter(s)
		sql.pullrequest.log.run( { eventId: data.id, number: pullrequest.number, action: action, parameter: '', timestamp: data.created_at } );
...
function handlerPushEvent( data/*, cache */ ) {
	...
	commits.forEach( c => {
		...
		sql.commit.insert.run( commitMeta );

	} );


	// log the actual action, current timestamp and parameter(s)
	sql.push.log.run( { eventId: data.id, id: id, timestamp: data.created_at } );
...
module.exports = {
	keyFn,
	filterFn,
	url: `/repos/${config.REPOSITORY}/events`,
	name: 'eventsLogger',
	workers: [
